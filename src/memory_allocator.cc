...
MemoryAllocator::MemoryAllocator(int id, size_t size)
	: allocator_id(id),  bins(k_num_of_bins), free_chunks(k_invalid_chunk_id), total_allocated_size(0) {
	total_size = RoundedBytes (size);
}

MemoryAllocator::~MemoryAllocator() {}

void MemoryAllocator::InitMemoryBins() {
...
void MemoryAllocator::InitMemoryChunks() {
...
std::shared_ptr<MemoryAllocator> MemoryAllocator::Create(int id, size_t size) {
	std::shared_ptr<MemoryAllocator> allocator = std::make_shared<MemoryAllocator>(id, size);
	allocator->InitMemoryBins();
	allocator->InitMemoryChunks();
    return allocator;
}

Offset MemoryAllocator::Allocate(size_t size) {
	if(size == 0) {
...
void MemoryAllocator::Deallocate(Offset offset) {
...
size_t MemoryAllocator::GetTotalAvaiableSize() {
	return total_size - total_allocated_size;
}

size_t MemoryAllocator::RoundedBytes(size_t bytes) {
	size_t rounded_bytes =
      (k_min_allocation_size *
	   ((bytes + k_min_allocation_size - 1) / k_min_allocation_size));
	return rounded_bytes;
}

int MemoryAllocator::BinNumberFromSize(size_t size) {
	uint64_t v = std::max<size_t>(size, 256) >> k_min_allocation_bits;
    int b = std::min(k_num_of_bins - 1, Log2FloorNonZero(v));
    return b;
}

size_t MemoryAllocator::SizeFromBinNumber(int bin_num) {
	return static_cast<size_t>(256) << bin_num;
}

Offset MemoryAllocator::FindChunkOffset(int bin_num, size_t rounded_size) {
...
void MemoryAllocator::FreeAndMaybeCoalesce(MemoryChunkId id) {
...
MemoryChunkId MemoryAllocator::AllocateChunk() {
...
MemoryChunkPtr MemoryAllocator::GetChunk(MemoryChunkId id) {
	return chunks[id];
}

void MemoryAllocator::DeleteChunk(MemoryChunkId id) {
	MemoryChunkPtr chunk = GetChunk(id);
	allocated_chunks.erase(chunk->offset);
	DeallocateChunk (id);
}

void MemoryAllocator::DeallocateChunk(MemoryChunkId id) {
	MemoryChunkPtr chunk = GetChunk(id);
	chunk->next = free_chunks;
	free_chunks = id;
}

void MemoryAllocator::InsertFreeChunkIntoBin(MemoryChunkId id) {
	MemoryChunkPtr chunk = GetChunk(id);
...
void MemoryAllocator::RemoveFreeChunkFromBin(MemoryChunkId id) {
	MemoryChunkPtr chunk = GetChunk(id);
	assert(chunk->is_free == true && chunk->bin_num != k_invalid_bin_num);
...
void MemoryAllocator::SplitChunk(MemoryChunkId id, size_t size) {
	MemoryChunkPtr chunk = GetChunk(id);
	assert(chunk->is_free == true && chunk->bin_num == k_invalid_bin_num);
...
void MemoryAllocator::MergeChunk(MemoryChunkId id1, MemoryChunkId id2) {
...
void MemoryAllocator::ShowDebug() {
...
// Implementation of MemoryBin

MemoryBin::MemoryBin(size_t size, std::weak_ptr<MemoryAllocator> allocator)
	: free_chunks(ChunkComparator(allocator)), size (size), parent_allocator (allocator) {}


MemoryChunkId MemoryBin::FindChunkIdLargerThanSize(size_t rounded_size) {
	auto allocator = parent_allocator.lock();
	for (auto itr = std::begin(free_chunks);
		itr != std::end(free_chunks); ++ itr) {
...
void MemoryBin::InsertMemoryChunk(MemoryChunkId id) {
	free_chunks.insert(id);
}

void MemoryBin::RemoveMemoryChunk(MemoryChunkId id) {
	free_chunks.erase(id);
}
