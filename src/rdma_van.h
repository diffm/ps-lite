...
protected:
    ...
    void Stop() override {
        ...
        /* FIXME(cjr) cause segment fault */
        for (auto it : connections_)
            rdma_disconnect(it.second);
        ...
        /* TODO(cjr) remember to free MemoryAllocator */
    ...
    int Bind(const Node &node, int max_retry) override {
        CHECK(rdma_create_id(event_channel_, &listener_,
        ...
        CHECK(rdma_listen(listener_, 10) == 0)
            << "listen RDMA connection failed";
    ...
    void Connect(const Node &node) override {
        ...
        CHECK(getaddrinfo(
                node.hostname.c_str(), std::to_string(node.port).c_str(),
                nullptr, &addr) == 0)
    ...
    void make_sge(struct ibv_sge *sge, void *addr,
    ...
    void WriteToPeer(struct connection *conn, int recver,
                     const DArray<char> &darray, void *addr, uint32_t rkey) {
        struct ibv_sge sge;
        ...
        make_sge(&sge, darray.data(), darray.size(), context_->rdma_mr->lkey);
    ...
    int SendMsg(const Message &msg) override {
        ...
        CHECK(conn->recv_msg->type == MSG_RES_REGION)
            << "receive message type != MSG_RES_REGION, "
            << conn->recv_msg->type;
        conn->sr_slots--;
        ...
        DArray<char> darray(meta_size);
        meta.SerializeToArray(darray.data(), meta_size);
        WriteToPeer(conn, recver_id, darray,
                    conn->recv_msg->data.mr.addr[0],
                    conn->recv_msg->data.mr.rkey);

        for (size_t i = 0; i < msg.data.size(); i++) {
            WriteToPeer(conn, recver_id, DArray<char>(msg.data[i]),
                        conn->recv_msg->data.mr.addr[i + 1],
                        conn->recv_msg->data.mr.rkey);
        ...
        do {
            ...
            if (wc.opcode == IBV_WC_RECV) {
                ...
                if (conn->recv_msg->type == MSG_WRITE_DONE)
                    break;
            }
        } while (1);
        ...
        while (!cq_poller_should_stop_) {
            ...
            if (conn->recv_msg->type == MSG_REQ_REGION) {
                ...
                for (int i = 0, length; (length = data.length[i]) != -1; i++) {
                    ...
                    offset = Memory::GetMemory()->allocator->Allocate(length);
                    CHECK(offset != brown::k_invalid_offset)
    ...
    int RecvMsg(Message *msg) override {
        ...
        /* 3. handle data from memory */
        addr = recv_msg.data.mr.addr[0];
        offset = (char *)addr - (char *)Memory::GetMemory()->ptr;

        header = &recv_msg.data.header;
        msg->meta.sender = header->sender;
        msg->meta.recver = my_node_.id;

        UnpackMeta((char *)addr, header->length[0], &msg->meta);

        recv_bytes += header->length[0];

        /* TODO(cjr) FIXME(cjr) one more copy here */
        for (int i = 1; header->length[i] != -1; i++) {
            addr = recv_msg.data.mr.addr[i];
            DArray<char> darray((char *)addr, header->length[i], true);
            SArray<char> sarray(darray);
            msg->data.push_back(sarray);

            recv_bytes += header->length[i];
        }
        
        Memory::GetMemory()->allocator->Deallocate(offset);
        return recv_bytes;
    }
...
private:
    ...
    void OnConnectRequest(struct rdma_cm_id *id) {
        ...
        CHECK(rdma_accept(id, &cm_params) == 0)
            << "accept RDMA connection failed";
    }
    ...
    void OnRouteResolved(struct rdma_cm_id *id) {
        struct rdma_conn_param cm_params;
        BuildConnParam(&cm_params);
        CHECK(rdma_connect(id, &cm_params) == 0)
            << "RDMA connect failed";
    }
    ...
    void BuildContext(struct ibv_context *verbs) {
        if (context_) {
            ...
            context_->cnt++;
            CHECK(ibv_resize_cq(context_->cq,
                                context_->cnt * (kRxDepth + kTxDepth)) == 0);
            return;
        }
    ...
    void RegisterMemory(struct connection *conn) {
        ...
        conn->send_msg_mr = ibv_reg_mr(
            context_->pd, conn->send_msg, rdma_msg_size,
            IBV_ACCESS_LOCAL_WRITE
        );
        CHECK(conn->send_msg_mr) << "register send_msg region failed";

        conn->recv_msg = (struct rdma_msg *)malloc(rdma_msg_size);
        conn->recv_msg_mr = ibv_reg_mr(
            context_->pd, conn->recv_msg, rdma_msg_size,
            IBV_ACCESS_LOCAL_WRITE
        );
