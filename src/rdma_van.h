...
class RDMAVan : public Van {
 ...
 protected:
  ...
  int SendMsg(const Message &msg) override {
    ...
    header->checksum = 0;
    header->checksum = checksum(reinterpret_cast<uint16_t *>(srmem.data()), srmem.size());
    ...
    do {
      ...
      CHECK(wc.opcode == IBV_WC_RECV_RDMA_WITH_IMM) << "不可能啊 opcode = " << wc.opcode;
  ...
  void PollCQ() {
    ...
    while (!cq_poller_should_stop_) {
      ...
      if (wc.opcode == IBV_WC_RECV_RDMA_WITH_IMM) {
        ...
        //struct rdma_write_header *header = (struct rdma_write_header *)(void *)recv_addr_[imm_data];
        //if (header->length[1] >= 400000)
        //  inspect(recv_addr_[imm_data] + sizeof(struct rdma_write_header) + header->length[0], 1025);
  ...
  int RecvMsg(Message *msg) override {
    ...
    inspect(reinterpret_cast<uint8_t *>(header), sizeof(*header) + header->length[0]);
